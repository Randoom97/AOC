...
def checkOrientations(scanner1: Scanner, scanner2s):
    for scanner2 in scanner2s:
        result = checkMatch(scanner1, scanner2)
        if result != None:
            return result
    return None

def genOrientations(scanner: Scanner):
    orientations = []
    # x orientations
    scannerc = scanner
    orientations.append(scannerc)
    scannerc = scannerc.rotateX()
    orientations.append(scannerc)
    scannerc = scannerc.rotateX()
    orientations.append(scannerc)
    scannerc = scannerc.rotateX()
    orientations.append(scannerc)
    scannerc = scannerc.rotateY().rotateY()
    orientations.append(scannerc)
    scannerc = scannerc.rotateX()
    orientations.append(scannerc)
    scannerc = scannerc.rotateX()
...
for beacons in input:
    scanners.append(genOrientations(Scanner(beacons)))

groundTruths = []
groundTruths.append(scanners.pop(0)[0])
...
for truth in groundTruths:
    beacons = beacons.union(truth.beacons)
print(len(beacons))
